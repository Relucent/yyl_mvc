...
public class JdbcDaoHelper {
	 ...
	 * @return 分页查询結果
	 */
	public static <T> Page<T> pagedQuery(CharSequence sql, List<Object> args, int start, int limit, RowMapper<T> rowMapper, JdbcTemplate jdbcTemplate,
			Dialect dialect) {
	...
	public static <T> Page<T> pagedQuery(CharSequence sql, List<Object> args, Pagination pagination, RowMapper<T> rowMapper,
		...
		return pagedQuery(sql.toString(), args.toArray(), pagination.getStart(), pagination.getLimit(), rowMapper, jdbcTemplate, dialect);
	}
