package yyl.mvc.util.bean.mapping;
...
public class BeanMapper {
    ...
    /**
     * Boolean. 本以为Boolean.valueOf()方法只能解析true,false的字符串 后来发现，只要字符串不是true，一律都会返回false，所以也没办法设置默认值了
     * @param value 需要转换的字符串
     * @return 转换结果
     */
     ...
     * @return 转换结果
     */
    public static Date getDate(String value, Date defaultValue, String datePattern) {
        try {
        ...
        for (Method method : srcClass.getMethods()) {
            ...
            if (isGetter(method)) {
                ...
                try {
                    ...
                    Method setter = destClass.getMethod(methodName, methodParam);
        ...
        for (Method method : srcClass.getMethods()) {
            ...
            if (isGetter(method)) {
                try {
                    ...
                    String methodName = getter2Setter(method.getName());
                    Method setter = destClass.getMethod(methodName, returnType);
                    setter.invoke(dest, result);
                } catch (Throwable ex) {
                    System.err.println(ex);
                }
    ...
    /**
     * 判断方法名是否符合getter. 1.方法名以get开头，并在3个字符以上 2.方法名以is开头，并在2个字符以上 3.方法没有参数 满足这三个条件，就被认为是getter
     * @param method 方法
     * @return boolean
     */
