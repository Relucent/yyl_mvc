...
public interface Connection {
	 ...
	 * @return this Connection, for chaining
	 */
	Connection proxy(Proxy proxy, String username, String password);
	 ...
	 * @return this Connection, for chaining
	 */
	Connection proxy(String host, int port, String username, String password);
	...
	/**
	 * Set the request timeouts (connect and read). If a timeout occurs, an IOException will be thrown. The default timeout
	 * is 3 seconds (3000 millis). A timeout of zero is treated as an infinite timeout.
	...
	/**
	 * Set the maximum bytes to read from the (uncompressed) connection into the body, before the connection is closed, and
	 * the input truncated. The default maximum is 1MB. A max size of zero is treated as an infinite amount (bounded only by
	 * your patience and the memory available on your machine).
	...
	/**
	 * Configures the connection to not throw exceptions when a HTTP error occurs. (4xx - 5xx, e.g. 404 or 500). By default
	 * this is <b>false</b>; an IOException is thrown if an error is encountered. If set to <b>true</b>, the response is
	 * populated with the error body, and the status message will reflect the error.
	 ...
	 * content-type will cause an IOException to be thrown. (This is to prevent producing garbage by attempting to parse a
	 * JPEG binary image, for example.) Set to true to force a parse attempt regardless of content type.
	 ...
	 * <p>
	 * By default this is <b>true</b>; all connections over HTTPS perform normal validation of certificates, and will abort
	 * requests if the provided certificate does not validate.
	 * </p>
	 * <p>
	 * Some servers use expired, self-generated certificates; or your JDK may not support SNI hosts. In which case, you may
	...
	/**
	 * Add an input stream as a request data paramater. For GETs, has no effect, but for POSTS this will upload the input
	 * stream.
	 * @param key data key (form item name)
	 * @param filename the name of the file to present to the remove server. Typically just the name, not path, component.
	 * @param inputStream the input stream to upload, that you probably obtained from a {@link java.io.FileInputStream}. You
	 *        must close the InputStream in a {@code finally} block.
	 ...
	 * "hello", "language", "Java", "language", "English");</code> creates a query string like:
	 * <code>{@literal ?name=hello&language=Java&language=English}</code>
	...
	/**
	 * Set a POST (or PUT) request body. Useful when a server expects a plain request body, not a set for URL encoded form
	 * key/value pairs. E.g.: <code><pre>connect(url)
	 * .requestBody(json)
	...
	interface Base<T extends Base<T>> {
		 ...
		 * <p>
		 * Response objects have a simplified cookie model. Each cookie set in the response is added to the response object's
		 * cookie key=value map. The cookie's path, domain, and expiry date are ignored.
		 * </p>
	...
	interface Request extends Base<Request> {
		 ...
		 * @return true if invalid content-types will be ignored; false (default) if they will cause an IOException to be
		 *         thrown.
		 */
